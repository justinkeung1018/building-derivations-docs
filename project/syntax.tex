...
Users may want to extend pre-existing proof systems with new syntactic constructs, or even define a new proof system from scratch. The web application must provide an interface for modifying syntax rules, check whether the syntax rules are defined sensibly, and be able to verify user input against an arbitrary set of syntax rules.

\Cref{syntax:syntax} defines the syntax in which a user expresses a syntax rule in the application. \Cref{syntax:ui} gives an overview of the parts of the user interface of the web application for modifying syntax rules. \Cref{syntax:parsing} describes the procedure for transforming an arbitrary set of syntax rules into intermediate internal representations, in preparation for verifying user input against the syntax rules.
